...
class tensor_core<subtensor_engine<T>>
  : public detail::tensor_expression<tensor_core<subtensor_engine<T>>, tensor_core<subtensor_engine<T>>> {
public:
    ...
    typename container_traits_type::const_reference, typename container_traits_type::reference>;
...
template <typename T> auto make_subtensor(const T& tens) { return subtensor<T>(tens); }
